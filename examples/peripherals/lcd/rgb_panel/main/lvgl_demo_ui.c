...
static lv_font_t const* font_normal = &lv_font_montserrat_14;

static lv_obj_t* create_scale_box(lv_obj_t* parent, char const* text1, char const* text2, char const* text3) {
...
static void scale1_indic1_anim_cb(void* var, int32_t v) {
    ...
    lv_obj_t const* card = lv_obj_get_parent(scale1);
...
void example_lvgl_demo_ui(lv_display_t* disp) {
    ...
    lv_anim_t anim;
    lv_anim_init(&anim);
    lv_anim_set_values(&anim, 20, 100);
    lv_anim_set_repeat_count(&anim, LV_ANIM_REPEAT_INFINITE);
    lv_anim_set_exec_cb(&anim, scale1_indic1_anim_cb);
    lv_anim_set_var(&anim, arc);
    lv_anim_set_duration(&anim, 4100);
    lv_anim_set_playback_duration(&anim, 2700);
    lv_anim_start(&anim);
    ...
    lv_anim_set_exec_cb(&anim, scale1_indic2_anim_cb);
    lv_anim_set_var(&anim, arc);
    lv_anim_set_duration(&anim, 2600);
    lv_anim_set_playback_duration(&anim, 3200);
    lv_anim_start(&anim);
    ...
    lv_anim_set_exec_cb(&anim, scale1_indic3_anim_cb);
    lv_anim_set_var(&anim, arc);
    lv_anim_set_duration(&anim, 2800);
    lv_anim_set_playback_duration(&anim, 1800);
    lv_anim_start(&anim);
}
